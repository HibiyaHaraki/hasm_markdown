...
- Logging format of the test script is fixed
  ...
  - React-Bootstrap is the required design system for all React pages and components. Use React-Bootstrap components for layout, navigation, buttons, forms, alerts, progress, dialogs, overlays, lists, and responsive behavior; use Bootstrap utility classes where appropriate. Keep HASM theme variables and custom CSS for editor-specific visuals and theme overrides, and do not add another UI library. Use accessible labels for unfamiliar icon-only controls.
